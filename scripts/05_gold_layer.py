"""
Script 05 - Couche Gold (Schema en etoile pour Power BI)
---------------------------------------------------------
1. Remplit toutes les tables de dimension (dim_*)
2. Remplit les tables de faits (fait_voyage, fait_co2)
3. Calcule les scores d'attractivite par gare

Resultat attendu : schema etoile complet pret pour Power BI
"""
...
transports = [
    (1, "Train",             "train",  2.4,   100, 0.10, 5),
    (2, "Voiture (seul)",    "voiture", 218.0,  90, 0.30, 1),
    (3, "Voiture (2 pers.)", "voiture", 109.0,  90, 0.15, 2),
    (4, "Avion",             "avion",  258.0, 500, 0.15, 1),
    (5, "Bus",               "bus",    103.0,  70, 0.05, 3),
    (6, "Velo",              "velo",     0.0,  15, 0.00, 5),
    (7, "Moto",              "moto",   191.0,  80, 0.15, 2),
]
df_trans = pd.DataFrame(transports, columns=[
    "id","nom","icone","co2_g_km","vitesse_moy_kmh","cout_moy_eur_km","eco_score"
])
with engine.connect() as conn:
...
# -- [5/7] DIM_GARE avec scores d'attractivite ---------------------------------

print("[5/7] dim_gare avec scores d'attractivite...")
...
try:
    df_mob = pd.read_sql(
        "SELECT id_gare_proche, COUNT(*) as nb FROM silver.mobilites WHERE id_gare_proche IS NOT NULL GROUP BY id_gare_proche",
        engine
    )
    mob_dict = dict(zip(df_mob['id_gare_proche'], df_mob['nb']))
except Exception:
    mob_dict = {}
...
# nb_mobilites par gare
df_dim_gare['nb_mobilites'] = df_dim_gare['id'].map(mob_dict).fillna(0).astype(int)

# Score attractivite (0-10)
...
def profil(s):
    ...
    if s >= 6:   return "Forte attractivite"
    if s >= 4:   return "Attractivite moyenne"
    if s >= 2:   return "Faible attractivite"
...
df_gold_gare = pd.DataFrame({
    ...
    "nb_mobilites"       : df_dim_gare['nb_mobilites'].astype(int),
...
print(f"  {len(df_gold_gare)} gares scorees")
...
df_gold_poi['score_popularite'] = df_poi_full['note_moyenne'].fillna(0)
...
id_temps_ete = pd.read_sql(
    "SELECT id FROM gold.dim_temps WHERE saison='Ete' AND est_weekend=true LIMIT 1",
    engine
).iloc[0]['id']
...
for _, gare in df_gare_gold.iterrows():
    for dep_nom, dep_lon, dep_lat, _ in villes_depart:
        ...
        nb_heb  = int(poi_q[poi_q['categorie'] == 'Hebergement']['n'].sum())  if len(poi_q) else 0
        nb_rest = int(poi_q[poi_q['categorie'] == 'Restauration']['n'].sum()) if len(poi_q) else 0
        nb_act  = int(poi_q[poi_q['categorie'].isin(['Sport & Loisirs','Nature','Culture','Loisirs'])]['n'].sum()) if len(poi_q) else 0
...
print("\nScript 05 termine - Couche Gold prete pour Power BI.")
